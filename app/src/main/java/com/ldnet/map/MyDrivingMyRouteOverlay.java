 ...
 * 导航路线图层类。
 */
public class MyDrivingMyRouteOverlay extends MyRouteOverlay {
     ...
     */
    public MyDrivingMyRouteOverlay(Context context, AMap amap, DrivePath path,
                                   LatLonPoint start, LatLonPoint end, List<LatLonPoint> throughPointList) {
        super(context);
        mContext = context;
        mAMap = amap;
        this.drivePath = path;
        startPoint = MyAMapUtil.convertToLatLng(start);
        endPoint = MyAMapUtil.convertToLatLng(end);
    ...
    private void initPolylineOptions() {
        ...
        mPolylineOptions.color(R.color.black).width(getRouteWidth());
    }
    ...
    private void colorWayUpdate(List<TMC> tmcSection) {
        ...
        mPolylineOptionscolor.add(MyAMapUtil.convertToLatLng(tmcSection.get(0).getPolyline().get(0)));
        ...
        for (int i = 0; i < tmcSection.size(); i++) {
            ...
            for (int j = 1; j < mployline.size(); j++) {
                mPolylineOptionscolor.add(MyAMapUtil.convertToLatLng(mployline.get(j)));
                colorList.add(color);
            }
        }
    ...
    public void setColor(){
        mPolylineOptionscolor.color(Color.BLUE);
    }
}