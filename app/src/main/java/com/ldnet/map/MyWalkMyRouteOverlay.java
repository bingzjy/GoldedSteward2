 ...
 * @since V2.1.0
 */
public class MyWalkMyRouteOverlay extends MyRouteOverlay {
	 ...
	 * @since V2.1.0
	 */
	public MyWalkMyRouteOverlay(Context context, AMap amap, WalkPath path,
                                LatLonPoint start, LatLonPoint end) {
		super(context);
		this.mAMap = amap;
		this.walkPath = path;
		startPoint = MyAMapServicesUtil.convertToLatLng(start);
		endPoint = MyAMapServicesUtil.convertToLatLng(end);
	}
	/**
	 * 添加步行路线到地图中。
	 * @since V2.1.0
    ...
    public void addToMap() {
        ...
        try {
            ...
            for (int i = 0; i < walkPaths.size(); i++) {
                ...
                LatLng latLng = MyAMapServicesUtil.convertToLatLng(walkStep
                        .getPolyline().get(0));
            ...
            Log.e("map","步行路线------addToMap2222222222");
            e.printStackTrace();
        }
        Log.e("map","步行路线------addToMap33333333");
    }
    ...
    private void addWalkPolyLine(LatLonPoint pointFrom, LatLonPoint pointTo) {
        addWalkPolyLine(MyAMapServicesUtil.convertToLatLng(pointFrom), MyAMapServicesUtil.convertToLatLng(pointTo));
    }
    ...
    private void addWalkPolyLines(WalkStep walkStep) {
        mPolylineOptions.addAll(MyAMapServicesUtil.convertArrList(walkStep.getPolyline()));
    }
